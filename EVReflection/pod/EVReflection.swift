...
final public class EVReflection {
    ...
    */
    ...
    public class func arrayFromJson<T>(_ theObject: NSObject? = nil, type: T, json: String?, conversionOptions: ConversionOptions = .DefaultDeserialize) -> [T] {
        ...
        do {
            if let jsonDic: [Dictionary<String, Any>] = try JSONSerialization.jsonObject(with: jsonData, options: JSONSerialization.ReadingOptions.mutableContainers) as? [Dictionary<String, Any>] {
    ...
    public class func decodeObjectWithCoder(_ theObject: EVObject, aDecoder: NSCoder, conversionOptions: ConversionOptions = .DefaultNSCoding) {
        ...
        for (key, _) in hasKeys {
            if aDecoder.containsValue(forKey: (key as? String)!) {
                let newValue: Any? = aDecoder.decodeObject(forKey: (key as? String)!) as Any?
    ...
    */
    ...
    public class func setBundleIdentifiers(_ classes: Array<AnyClass>) {
        ...
        for aClass in classes {
            if let bundle: Bundle = Bundle(for: aClass) {
    ...
    /**
     Helper function to convert an Any to Any
     ...
     - returns: The value where the Any is converted to Any plus the type of that value as a string
     */
    public class func valueForAny(_ parentObject: Any? = nil, key: String? = nil, anyValue: Any, conversionOptions: ConversionOptions = .DefaultDeserialize, isCachable: Bool = false, parents: [NSObject] = []) -> (value: Any, type: String, isObject: Bool) {
        ...
        if mi.displayStyle == .optional {
            if mi.children.count == 1 {
                ...
                if "\(type(of: (theValue) as Any))".hasPrefix("_TtC") {
                  valueType = "\(theValue)".components(separatedBy: " ")[0]
                } else {
                    valueType = "\(type(of: (theValue) as Any))"
                }
        ...
        if mi.displayStyle == .enum {
            ...
            if let value = theValue as? EVRawString {
                return (value.rawValue as Any, "\(mi.subjectType)", false)
    ...
    public class func valueForAnyDetail(_ parentObject: Any? = nil, key: String? = nil, theValue: Any, valueType: String) -> (value: Any, type: String, isObject: Bool) {
        ...
        if theValue is Date {
            return (theValue as! Date as Any, "NSDate", false)
        }
    ...
    public static func setObjectValue<T>(_ anyObject: T, key: String, theValue: Any?, typeInObject: String? = nil, valid: Bool, conversionOptions: ConversionOptions = .DefaultDeserialize, parents: [NSObject] = []) where T: NSObject {
        ...
        if (typeInObject == "String" || typeInObject == "NSString") && type == "NSNumber" {
            if let convertedValue = value as? NSNumber {
                value = convertedValue.stringValue as Any
            }
    ...
    */
     ...
     */
    fileprivate static func dictionaryAndArrayConversion(_ anyObject: NSObject, key: String, fieldType: String?, original: NSObject?, theDictValue: Any?, conversionOptions: ConversionOptions = .DefaultDeserialize) -> (Any?, Bool) {
    ...
    fileprivate class func convertDictionaryForJsonSerialization(_ dict: NSDictionary, theObject: NSObject) -> NSDictionary {
        ...
        for (key, value) in dict {
            dict2.setValue(convertValueForJsonSerialization(value as Any, theObject: theObject), forKey: key as? String ?? "")
        }
        return dict2
    }
     ...
     */
    fileprivate class func convertValueForJsonSerialization(_ value: Any, theObject: NSObject) -> Any {
        switch value {
        ...
        case let date as Date:
            return (getDateFormatter().string(from: date) as Any? ?? "" as Any)
